...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Cookie
    const tipoCookie = document.cookie.split("; ").find(r => r.startsWith("Id_tipo_usuario="));
    const tipo = tipoCookie ? tipoCookie.split("=")[1].trim() : null;

    // Regex
    const regex = {
        telefono: /^\d{3}-\d{4}-\d{3}$/,
        correo: /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|mx)$/,
        grupo: /^[1-9][A-Z]$/
    };
    ...
    document.getElementById("telefono_administrador").addEventListener("input", e => {
    ...
    function resetBotones(editando) {
    ...
    function guardar(datos) {
        fetch("guardar_datos.php", {
            ...
            .then(r => r.json())
            .then(r => lanzarToast(r.success ? "Datos guardados correctamente" : "Error al guardar", r.success ? "exito" : "error"))
    ...
    btn.guardar.addEventListener("click", () => {
        ...
        let datos = {};
        if (esAdmin()) {
            datos = Object.fromEntries(camposRol().map(id => [id, getVal(id)]));
        } else {
            datos = {
