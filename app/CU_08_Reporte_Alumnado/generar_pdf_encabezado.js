...
async function dibujarEncabezado(doc, cfg) {
    ...
    doc.text('Licenciatura: ', margin + colMitad, y);
    const labelLicW = doc.getTextWidth('Licenciatura: ');
    if (cfg.carrera) doc.text(cfg.carrera, margin + colMitad + labelLicW, y);
    doc.line(margin + colMitad + labelLicW, y + 0.5, pageW - margin, y + 0.5);

    // ── Fila 2: Periodo | Grupo | Docente ───────────────────────────────────────
    y += 7;
    ...
    // Grupo
    const grupoX = margin + colTercio;
    doc.text('Grupo: ', grupoX, y);
    const labelGruW = doc.getTextWidth('Grupo: ');
    if (cfg.grupo) doc.text(cfg.grupo, grupoX + labelGruW, y);
    doc.line(grupoX + labelGruW, y + 0.5, grupoX + colTercio - 2, y + 0.5);
 ...
 * );
 */
export const handleImprimirPDF = async function (elTabla, cfgIndex = 0, titulo, codigo, revision, fecha, version, facultad, carrera, periodo, grupo, docente) {
    ...
    const headers = [...tablaClonada.querySelector('#titulos-tabla').querySelectorAll('th')]
        .map(th => th.textContent.trim())
        .filter(t => t !== '');

    // Extraer filas desde #Tabla
    const data = [...tablaClonada.querySelector('#Tabla').querySelectorAll('tr')].map(row =>
