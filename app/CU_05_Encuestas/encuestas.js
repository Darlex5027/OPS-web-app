...
let idAlumnoContexto = (idTipoUsuario === 2) ? (getCookie('Id_alumno') || "1") : null;
            ...
            listaDiv.innerHTML = encuestas.map(e => `
                <div class="card-encuesta">
                    <div class="card-body">
                        ...
                        <h5>${e.Nombre}</h5>
                        <p>${e.Descripcion}</p>
                        <button type="button" class="btn-responder" onclick="abrirEncuesta(${e.Id_encuesta}, '${e.Nombre}', '${e.Descripcion}', '${idAlumnoContexto}')">
                            Responder encuesta
                        </button>
                    </div>
                </div>
...
async function cargarAlumnosCoordinador() {
    try {
        ...
        // El backend ya debe retornar SOLO alumnos con pendientes según documento
        ...
        if (alumnosConPendientes.length === 0) {
            ...
            // Según documento por cada alumno:
            // - Mostrar nombre completo
            // - Mostrar contador de encuestas pendientes
                    ...
                    <div id="acordeon-${a.Id_alumno}" class="lista-encuestas-desplegable" style="display: none;">
                        ${a.encuestas.map(e => `
                            <div class="subcard-encuesta">
                                <p><strong>Servicio:</strong> ${e.NombreServicio || e.servicio_nombre || 'N/A'}</p>
                                <p><strong>Encuesta:</strong> ${e.Nombre}</p>
                                <p><strong>Descripción:</strong> ${e.Descripcion}</p>
                                <button type="button" class="btn-responder-admin" onclick="abrirEncuesta(${e.Id_encuesta}, '${e.Nombre}', '${e.Descripcion}', ${a.Id_alumno})">
                                    Responder encuesta
                                </button>
                            </div>
                        `).join('')}
...
function renderizarPregunta(p) {
    ...
    const asterisco = esObligatoria ? '<span class="obligatorio">*</span>' : '';
    ...
    return `
        <div class="pregunta-wrapper" id="wrapper_${p.Id_pregunta}" ${obligatoriaAttr}>
            <p class="pregunta-header">
                <strong>${p.Pregunta}</strong> ${asterisco}
            </p>
...
document.getElementById('form-encuesta').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Resetear contexto y volver a la lista
            if (idTipoUsuario === 2) {
